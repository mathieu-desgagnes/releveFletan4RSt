...
lecture_stations <- function(source, destination){
  ...
  #file.info(source)$mtime; nrow(stations.init)
      if(FALSE){ #a valider plus tard
        ## 3Pn
        stations.init.3Pn <- read.csv2('C:/gccode/releve4RST/Donnees/donneesReleveFletan3Pn_stations.csv', stringsAsFactors=FALSE, dec='.')
        file.info('C:/gccode/releve4RST/Donnees/donneesReleveFletan3Pn_stations.csv')$mtime; nrow(stations.init.3Pn)
        stations.init.3Pn$releve <- 'opano3Pn'
        ## pêche dirigée
        stations.init.supp <- read.csv2('C:/gccode/releve4RST/Donnees/donneesReleveFletanDirigee_stations.csv', stringsAsFactors=FALSE, dec='.')
        stations.init.supp[which(stations.init.supp$profondeurFin=='Na'), 'profondeurFin'] <- NA
        stations.init.supp$profondeurFin <- as.numeric(stations.init.supp$profondeurFin)
        file.info('C:/gccode/releve4RST/Donnees/donneesReleveFletanDirigee_stations.csv')$mtime; nrow(stations.init.supp)
        stations.init.supp$releve <- 'stSupp'
        stations.init <- rbind(stations.init, stations.init.3Pn, stations.init.supp); nrow(stations.init)
      }
      ##
  lesquels <- which(is.na(as.numeric(stations.init$latDebDD)) | is.na(as.numeric(stations.init$longDebDD)) |
                    is.na(as.numeric(stations.init$latFinDD)) | is.na(as.numeric(stations.init$longFinDD)))
  print(paste('Problème avec la position de la station:', stations.init[lesquels,'cleStation']))
  if(FALSE){
    stations.init[which(is.na(as.numeric(stations.init$latDebDD)) | is.na(as.numeric(stations.init$longDebDD)) |
                        is.na(as.numeric(stations.init$latFinDD)) | is.na(as.numeric(stations.init$longFinDD))),
                  c('noStation','cleStation','annee','latDebDD','longDebDD','latFinDD','longFinDD')]
  }
  ##
  stations.init$X <- apply(stations.init[,c('longDebDD','longFinDD')], 1, mean, na.rm=TRUE)
  stations.init$Y <- apply(stations.init[,c('latDebDD','latFinDD')], 1, mean, na.rm=TRUE)
  ##
  ## mettre les profondeurs sur la même unité
  table(stations.init$profondeurUnit, useNA='always')
  stations.init$profDeb <- NA
  stations.init$profDeb[which(stations.init$profondeurUnit=='metre')] <- stations.init$profondeurDebut[which(stations.init$profondeurUnit=='metre')]
  stations.init$profDeb[which(stations.init$profondeurUnit=='brasse')] <- 1.8288 *
    stations.init$profondeurDebut[which(stations.init$profondeurUnit=='brasse')]
  stations.init$profDeb[which(stations.init$profondeurUnit%in%c('pied'))] <- 0.3048 *
    stations.init$profondeurDebut[which(stations.init$profondeurUnit%in%c('pied'))]
  stations.init$profFin <- NA
  stations.init$profFin[which(stations.init$profondeurUnit=='metre')] <- stations.init$profondeurFin[which(stations.init$profondeurUnit=='metre')]
  stations.init$profFin[which(stations.init$profondeurUnit=='brasse')] <- 1.8288 *
    stations.init$profondeurFin[which(stations.init$profondeurUnit=='brasse')]
  stations.init$profFin[which(stations.init$profondeurUnit%in%c('pied'))] <- 0.3048 *
    stations.init$profondeurFin[which(stations.init$profondeurUnit%in%c('pied'))]
  stations.init$profMoy <- apply(stations.init[,c('profDeb','profFin')],1,mean,na.rm=TRUE)
  ##
  ...
  if(FALSE){
    profRaster <- function(st){
          ...
          rast20 <- raster('C:/gccode/pbsmapping/input/bathy_DAISS/gulf20m_2020May26_ascii/gulf20m_mathieuDesgagnes.grd')
          ## proj4string(rast20)
        ...
        lesquels <- which(!is.na(as.numeric(stations.init$latDebDD)) & !is.na(as.numeric(stations.init$longDebDD))) # peut pas calculer si manque une donnee
        stations.init[lesquels,'profDebBathy'] <- -profRaster(st=as.data.frame(list(X=stations.init[lesquels,'longDebDD'],
                                                                                    Y=stations.init[lesquels,'latDebDD'])))
        lesquels <- which(!is.na(as.numeric(stations.init$latFinDD)) & !is.na(as.numeric(stations.init$longFinDD))) # peut pas calculer si manque une donnee
        stations.init[lesquels,'profFinBathy'] <- -profRaster(st=as.data.frame(list(X=stations.init[lesquels,'longFinDD'],
                                                                                    Y=stations.init[lesquels,'latFinDD'])))
        stations.init$profMoyBathy <- apply(stations.init[,c('profDebBathy','profFinBathy')],1,mean,na.rm=TRUE)
  }
  ##
  ##
  ## Estimation des fuseau horaires manquants
  stations.init[is.na(stations.init$fuseauHoraire) & stations.init$association=='FFAW', 'fuseauHoraire'] <- "UTC-2.5"
  stations.init[is.na(stations.init$fuseauHoraire) & stations.init$association=='ACPG', 'fuseauHoraire'] <- "UTC-4"
  stations.init[is.na(stations.init$fuseauHoraire) & stations.init$association=='GNSFPB', 'fuseauHoraire'] <- "UTC-3"
  ...
  stations.init[which(stations.init$fuseauHoraire=="UTC-4"), 'fuseauTxt'] <- "-04:00"
  stations.init[which(stations.init$fuseauHoraire=="UTC-3"), 'fuseauTxt'] <- "-03:00"
  stations.init[which(stations.init$fuseauHoraire=="UTC-2.5"), 'fuseauTxt'] <- "-02:30"
  ##
  ...
  desc_text <- ifelse(
    ...
    sprintf("%04d-%02d-%02d", stations.init$annee, stations.init$mois, stations.init$jour),
    sprintf("%04d-%02d-%02dT%02d:%02d:00%s",
            stations.init$annee, stations.init$mois, stations.init$jour,
  ...
  desc_text_date <- sprintf("%04d-%02d-%02d",
                            ...
                            stations.init$mois,
                            stations.init$jour)
  desc_text_datetime <- ifelse(!is.na(stations.init$heureDescente) &
  ...
  ##
  if(FALSE){
        stations.init[1183:1193,c('fuseauHoraire','dateHeureDescenteLocale','dateHeureDescente','annee','mois','jour','heureDescente','minuteDescente')]
        stations.init[1183:1193,c('fuseauHoraire','dateHeureRemonteeLocale','dateHeureRemontee','heureRemonte','minuteRemonte','immersionHeure','immersionMinute','annee',
                                  'mois','jour')]
        stations.init[1183:1193,c('fuseauHoraire','dateHeureDescenteLocale','dateHeureDescente', 'dateHeureRemonteeLocale','dateHeureRemontee','immersion','immersionHeure',
                                  'immersionMinute','annee','mois','jour')]
  }
      ##
      ...
      if(FALSE){
        ...
        for(i in 1:nrow(stations.choix)){
          if(all(!is.na(stations.choix[i,c('longFinDD','latFinDD')])) & all(!is.na(stations.choix[i,c('longDebDD','latDebDD')]))){
            stations.choix[i,'longueurTraitKm'] <- spDistsN1(as.matrix(stations.choix[i,c('longFinDD','latFinDD')]),
                                                             as.matrix(stations.choix[i,c('longDebDD','latDebDD')]), longlat=TRUE)
          }
        }
      }
      ##
      ...
      for(i in 1:nrow(stations.choix)){
        ...
        stations.choix[i,'heureNauticalDusk'] <- data.table::as.ITime(temp$nauticalDusk)
      }
