...
lecture_recaptures <- function(origine, destination) {
  ...
  recap.init$tEnMerCut <- cut(
    recap.init$tEnMer,
    breaks = c(0, 100, 200, 300, 400, 600, 800, 1200, 1600, 2200, 2800, 3600)
  )
  recap.init$tEnMer.an <- cut(
    recap.init$tEnMer,
    breaks = seq(-250, 4000, by = 365)
  )
  recap.init$tEnMer.demiAn <- cut(
    recap.init$tEnMer,
    breaks = seq(-250, 4000, by = 365 / 2)
  )
  recap.init$tEnMer.deuxAns <- cut(
    recap.init$tEnMer,
    breaks = seq(-250, 4000, by = 365 * 2)
  )
  ##
  recap.init[is.na(recap.init$tailleCm), 'tailleCm'] <- recap.init[
    is.na(recap.init$tailleCm),
    'taillePo'
  ] *
    2.54
  ##
  ...
  load(
    file.path(
      ...
      'input',
      anneeCourante,
      'longueursPlusMeta.RData'
    ),
    verbose = 1
  )
  nrow(longueurs.temp)
  ##
  tag.temp <- subset(longueurs.temp, !is.na(tag_1) | !is.na(tag_2))
  nrow(tag.temp)
  tag.temp$nbTagPose <- as.numeric(!is.na(tag.temp$tag_1)) +
    as.numeric(!is.na(tag.temp$tag_2))
  tag <- tag.temp[, c(
    'cleStation',
    'clePoisson',
    'annee',
  ...
  ##
  if (TRUE) {
    #identifie les stations à considérer comme "représentatives" de la population, aller chercher dans le fichier des stations
    tag$pourCalculF <- FALSE
    length(which(
      tag$annee >= 2017 & tag$noStation <= 120 & tag$longueur_cm >= 77
    )) /
    ...
    tag[
      which(tag$annee >= 2017 & tag$longueur_cm >= 77),
      'pourCalculF.prime'
    ] <- TRUE
  }
  ##
  ...
  tag[which(!is.na(lesquels)), 'anneeRecap'] <- recap.init[
    na.omit(lesquels),
    'an_recap'
  ]
  ##
  tag[which(!is.na(lesquels)), 'longueur_cmRecap'] <- recap.init[
    na.omit(lesquels),
    'taille_cm'
  ]
  tag[which(!is.na(lesquels)), 'nbTagRecap'] <- recap.init[
    na.omit(lesquels),
    'nbTagRecap'
  ]
  ## tag$nbTagPerdu <- recap.init[match(tag$clePoisson, recap.init$clePoisson),'nbPerdu']
  tag$nbTagPerdu <- tag$nbTagPose - tag$nbTagRecap
  tag[which(!is.na(lesquels)), 'tEnMer'] <- recap.init[
    na.omit(lesquels),
    'tEnMer'
  ]
  tag$nbAnEnMer <- tag$anneeRecap - tag$annee
  tag[which(!is.na(lesquels)), 'tEnMer.an'] <- recap.init[
    na.omit(lesquels),
    'tEnMer.an'
  ]
  ##
  ...
  if (FALSE) {
    #validation visuelle des la séparation des temps en mer en années
    par(mfrow = c(1, 2))
    hist(tag$tEnMer, breaks = seq(0, 4100, by = 365 / 12))
    abline(v = -240 + 365 * 0:12, col = 2)
    text(x = -60 + 365 * 0:12, y = par('usr')[4] * 0.95, labels = 0:12)
    hist(
      subset(tag, nbTagPerdu == 1)$tEnMer,
      breaks = seq(0, 4100, by = 365 / 12),
      col = 2,
      add = TRUE
    )
    hist(
      subset(tag, tag$pourCalculF)$tEnMer,
      breaks = seq(0, 4100, by = 365 / 12)
    )
    abline(v = -240 + 365 * 0:10, col = 2)
    text(x = -60 + 365 * 0:10, y = par('usr')[4] * 0.95, labels = 0:10)
    hist(
      subset(tag, tag$pourCalculF & nbTagPerdu == 1)$tEnMer,
      breaks = seq(0, 4100, by = 365 / 12),
      col = 2,
      add = TRUE
    )
  ...
  perteTag <- na.omit(tagRelPal[
    tagRelPal$nbTagPose > 1,
    c('nbTagPose', 'nbTagRecap', 'nbTagPerdu', 'nbAnEnMer')
  ])
  nrow(perteTag)
  write.csv2(
    perteTag,
    file = paste(destination, '_perteTag.csv'),
    row.names = FALSE
  )
  tagF <- subset(tag[order(tag$date), ], pourCalculF == TRUE)
  write.csv2(
    tagF,
    file = paste0(destination, '_bdTagPourF.csv'),
    row.names = FALSE
  )
